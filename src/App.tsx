...
function App() {
  ...
  const goToGuest = () => setMode('guest')
  const startNewListing = () => { setDraft(emptyDraft); setEditingId(null); setStep(0); setMode('host'); setHostView('wizard') }
  ...
  const removeListing = (id: number) => setListings((items) => items.filter((item) => item.id !== id))
...
function HostDashboard({ listings, onCreate, onEdit, onRemove }: { listings: Listing[]; onCreate: () => void; onEdit: (listing: Listing) => void; onRemove: (id: number) => void }) {
  return <>
    <section className="hero host-hero"><div className="hero-copy"><p className="eyebrow"><span className="eyebrow-dot" /> HOSTING ON LANDLEASE</p><h1>List your land.<br /><em>Earn every season.</em></h1><p className="hero-description">Reach verified farmers searching for land right now. You set the price, the term and who leases it.</p><div className="trust-row"><span><ShieldCheck size={16} /> You control the terms</span><span><Check size={16} /> Free to list</span></div><button className="dark-button host-cta" onClick={onCreate}><Plus size={15} /> List new land</button></div><div className="hero-art"><div className="hero-image" /></div></section>
    <section className="content-section">
      ...
      {listings.length === 0
        ...
        : <div className="listing-grid">{listings.map((listing) => <HostListingCard key={listing.id} listing={listing} onEdit={() => onEdit(listing)} onRemove={() => onRemove(listing.id)} />)}</div>}
    </section>
  </>
}

function HostListingCard({ listing, onEdit, onRemove }: { listing: Listing; onEdit: () => void; onRemove: () => void }) {
  return <article className="listing-card host-listing-card"><div className="listing-image-wrap"><img src={listing.image} alt={listing.name} /><span className="live-badge">Live</span></div><div className="listing-body"><div className="listing-title-row"><div><h3>{listing.name}</h3><p><MapPin size={13} /> {listing.area}</p></div></div><div className="listing-meta"><span>{listing.size}</span><span className="meta-dot" /><span>{listing.crop}</span></div><div className="listing-footer"><div><strong>{listing.price}</strong><small>{listing.detail}</small></div><div className="host-card-actions"><button className="icon-button" onClick={onEdit} aria-label="Edit listing"><Pencil size={15} /></button><button className="icon-button" onClick={onRemove} aria-label="Remove listing"><Trash2 size={15} /></button></div></div></div></article>
}
